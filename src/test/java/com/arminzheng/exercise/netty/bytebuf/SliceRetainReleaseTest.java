...
/**
 * #### slice 【零拷贝】的体现之一， 对原始 ByteBuf 进行切片成多个 ByteBuf， 切片后的 ByteBuf 并没有发生内存复制，还是使用原始 ByteBuf 的内存，
 * 切片后的 ByteBuf 维护独立的 read，write 指针
 *
 * <p>注意点：1. 切片的 ByteBuf 将已限制最大容量，不得追加 2. ByteBuf 和 slice 分别维护自己独立的 read，write 指针 3. 对 ByteBuf
 * release【引用计数-1】操作之后，不得使用 slice切片的数据，除非 先使用 retain【引用计数+1】 4. 正确用法：对单个切片进行 retain 和 release
 * 成对处理，不会乱 5. 【--- 无论 对 原buf 还是 slice 得到的 ByteBuf引用计数 都是一样的，都是对物理内存进行引用计数 ---】
 *
 * <p>duplicate(也是零拷贝的提现，没有max限制，读写指针独立): 如果slice之后的进行duplicate，那么可以写数据，并且会覆盖原有内存地址的数据
 *
 * <p>copy 深拷贝
 */
public class SliceRetainReleaseTest {
