 ...
 * CompositeByteBuf
 *
 * <p>1. 零拷贝 的体现之一，可以将多个 ByteBuf 合并为一个逻辑上的 ByteBuf，避免拷贝
 *
 * <p>2. 与slice相反
 *
 * <p>缺点：更复杂的维护 还要注意 retain release
 */
@Slf4j
public class ByteBufCompositeBufferTest {
    ...
    public void writeBytes_copy() {
        ...
        System.out.println(
                buf1.getClass()); // : class io.netty.buffer.PooledUnsafeDirectByteBuf  池化直接内存

        // readableBytes 返回可读取的字节数
        final ByteBuf bufferWriteCopy =
                ByteBufAllocator.DEFAULT.buffer(buf1.readableBytes() + buf2.readableBytes());
