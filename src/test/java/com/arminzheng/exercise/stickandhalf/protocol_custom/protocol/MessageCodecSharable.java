package com.arminzheng.exercise.stickandhalf.protocol_custom.protocol;

import com.arminzheng.exercise.stickandhalf.protocol_custom.message.Message;
...
/**
 * 自定义消息编解码类 (支持@Sharable, 写这个类肯定的认为, 上一个处理器是帧解码器，所以不用考虑半包黏包问题，直接解码拿数据)
 *
 * <p>父类 MessageToMessageCodec 认为是完整的信息 【所以必须保证上一个处理器时 帧解码器】
 *
 * <p>相当于两个handler合二为一，既能入站 也能做出站处理
 *
 * <pre>
...
public class MessageCodecSharable
        extends MessageToMessageCodec<ByteBuf, Message> { // 注意这里Message是自定义类
